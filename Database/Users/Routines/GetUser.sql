CREATE OR REPLACE FUNCTION users.GetUser(_player_id INT, _is_join BOOLEAN)
    RETURNS JSON
    SECURITY DEFINER
    LANGUAGE plpgsql
AS
...
BEGIN
    IF _player_id IS NOT NULL AND _is_join = false THEN
        UPDATE users.player SET status = 'offline' WHERE player_id = _player_id;
        RETURN NULL;
    END IF;

    IF _player_id IS NOT NULL AND _is_join = true THEN
        UPDATE users.player SET status = 'online' WHERE player_id = _player_id;
    END IF;
